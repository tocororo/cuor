...
def _create_record(client):
    ...
    url = "https://localhost:5000/organizations/"
    response = client.post(url, data=json.dumps(data), headers=headers)
    assert response.status_code == 201
    current_search.flush_and_refresh("organizations")
...
def _get_record(client, pid_value):
    """Get record by PID."""
    url = "https://localhost:5000/organizations/{}".format(pid_value)
...
def test_files_creation_deletion(client, location):
    ...
    url = "https://localhost:5000/organizations/{}/files/test.jpg".format(pid_value)
    ...
    files_link = 'https://localhost:5000/organizations/1/files'
